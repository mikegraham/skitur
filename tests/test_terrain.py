...
def dem(terrain_loader):
    ...
    return terrain_loader.load(45.30, 45.40, -121.75, -121.35, padding=0.01)
...
def test_extent_too_large_rejected(terrain_loader):
    """Requests exceeding 1x1 degree should raise ExtentTooLargeError."""
    with pytest.raises(ExtentTooLargeError, match="too large"):
        terrain_loader.load(40.0, 41.5, -122.0, -121.0, padding=0.01)


def test_extent_limit_checks_raw_bounds_not_padded(terrain_loader):
    ...
    with pytest.raises(ExtentTooLargeError):
        terrain_loader.load(40.0, 41.01, -122.0, -121.5, padding=0.01)

    # 0.5 deg lat span: under limit even though padding makes it 0.52
    terrain_loader.load(45.30, 45.40, -121.75, -121.65, padding=0.01)
...
def test_dem_fallback_uses_second_source_on_no_coverage(terrain_loader):
    ...
    def mock_fetch(self, source, lat_min, lat_max, lon_min, lon_max):
    ...
    with patch("skitur.terrain._source_covers_bounds", side_effect=mock_covers), \
         patch.object(TerrainLoader, "_fetch", mock_fetch):
        dem = terrain_loader.load(45.30, 45.35, -121.70, -121.65, padding=0.01)

    assert fetch_log == [_DEM_SOURCES[1].name]
    assert isinstance(dem, Terrain)


def test_dem_fallback_raises_when_all_sources_fail(terrain_loader):
    ...
    with patch("skitur.terrain._source_covers_bounds", return_value=False):
        with pytest.raises(NoDEMCoverage, match="No DEM source covers"):
            terrain_loader.load(45.30, 45.35, -121.70, -121.65, padding=0.01)
